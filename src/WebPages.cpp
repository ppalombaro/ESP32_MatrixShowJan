...
String WebPages::buildControlPage(ContentManager* content) {
    ...
    for (const auto& theme : themes) {
        ...
        for (const auto& item : items) {
            ...
            html += "</p>";
            html += "<button onclick='preview(" + String(item.id) + ")'>Preview</button>";
            html += "</div>";
        }
...
String WebPages::buildSchedulePage(ContentManager* content) {
    ...
    html += "<p><strong>Status:</strong> " + String(content->isRandomModeEnabled() ? "ENABLED" : "DISABLED") + "</p>";
    ...
    if (content->isRandomModeEnabled()) {
        html += "<button class='control-btn danger-btn' onclick='toggleRandom(false)'>Disable Random Mode</button>";
    } else {
        html += "<button class='control-btn' onclick='toggleRandom(true)'>Enable Random Mode</button>";
    }
    html += "</div>";
