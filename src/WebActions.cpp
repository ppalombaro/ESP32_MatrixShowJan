...
void WebActions::attach() {
    ...
    server->on("/api/render", HTTP_GET, [this]() {
        ...
        String response = success ? "Ã¢Å“â€¦ Rendering: " + (item ? item->name : "Unknown") : "Ã¢ÂÅ’ Content not found";
    ...
    server->on("/api/random/interval", HTTP_GET, [this]() {
        ...
        server->send(200, "text/plain", "Ã¢Å“â€¦ Interval set to " + String(intervalMs) + " ms");
    });
    ...
    server->on("/api/random/filter", HTTP_GET, [this]() {
        ...
        String response = theme.length() > 0 ? 
                         "Ã¢Å“â€¦ Filter set to: " + theme : 
                         "Ã¢Å“â€¦ Filter cleared (ALL THEMES)";
    ...
    server->on("/api/scheduler/enable", HTTP_GET, [this]() {
        ...
        server->send(200, "text/plain", "Ã¢Å“â€¦ Scheduler ENABLED");
    });
    
    server->on("/api/scheduler/disable", HTTP_GET, [this]() {
        contentMgr->enableScheduler(false);
        server->send(200, "text/plain", "Ã¢â€ºâ€ Scheduler DISABLED");
    });
    ...
    server->on("/api/logs/clear", HTTP_GET, [this]() {
        ...
        server->send(200, "text/plain", "Ã¢Å“â€¦ Logs cleared");
    });
    ...
    server->on("/api/theme/set", HTTP_GET, [this]() {
        ...
        server->send(200, "text/plain", "Ã¢Å“â€¦ Theme set to " + String(themeId));
    });
}
